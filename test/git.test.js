...
describe('#git', function () {
  ...
  describe('versions', function () {
    ...
    it('shall get versions from remote git', function () {
      return versions(
        'hosted-git-info',
        'github:npm/hosted-git-info#v1.2.0'
      ).then(({ versions }) => {
        log(versions)
    ...
    it('shall get semver versions from remote git', function () {
      return versions(
        'hosted-git-info',
        'github:npm/hosted-git-info#semver:~1.5.1'
      ).then(({ versions, range }) => {
        log(versions, range)
        assert.ok(Array.isArray(versions), 'shall be an Array')
        assert.ok(versions.includes('1.1.0'), 'shall include v1.1.0')
    ...
    it('shall get semver versions from remote git using short semver', function () {
      return versions(
        'hosted-git-info',
        'github:npm/hosted-git-info#semver:^1'
      ).then(({ versions, range }) => {
        log(versions, range)
        assert.ok(Array.isArray(versions), 'shall be an Array')
        assert.ok(versions.includes('1.1.0'), 'shall include v1.1.0')
    ...
    it('shall return an error if module not found', function () {
      return versions('hosted-git-info', 'github:test/test/test').then(
        (res) => {
          log('%s', res.error)
          assert.ok(
            /fatal: unable to access|fatal: repository/.test(res.error),
